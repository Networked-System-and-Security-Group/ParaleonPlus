...
TypeId RdmaHw::GetTypeId (void)
{
	static TypeId tid = TypeId ("ns3::RdmaHw")
		...
		.AddAttribute("AlphaResumInterval",
				...
				MakeDoubleChecker<double>())
		.AddAttribute("InitialAlphaValue",
				"Initial alpha applied on the first CNP",
				DoubleValue(1023.0),
				MakeDoubleAccessor(&RdmaHw::m_initialAlphaValue),
				MakeDoubleChecker<double>())
		.AddAttribute("DceTcpG",
				"Alpha EWMA gain denominator term used by DCQCN",
				UintegerValue(1019),
				MakeUintegerAccessor(&RdmaHw::m_dce_tcp_g),
				MakeUintegerChecker<uint32_t>())
		.AddAttribute("RpgGd",
				"DCQCN rate decrease exponent",
				UintegerValue(11),
				MakeUintegerAccessor(&RdmaHw::m_rpg_gd),
				MakeUintegerChecker<uint32_t>())
		.AddAttribute("MinDecFac",
				"Minimum multiplicative decrease factor",
				DoubleValue(0.5),
				MakeDoubleAccessor(&RdmaHw::m_min_dec_fac),
				MakeDoubleChecker<double>())
		.AddAttribute("RateAI",
...
RdmaHw::RdmaHw(){
	m_dcqcnPlusEnabled = false;
	m_dcqcnPlusMinCnpIntervalUs = 45;
	m_dcqcnPlusMinTauUs = 50;
	m_dcqcnPlusScanIntervalUs = 1;
	m_dcqcnPlusEntryTimeoutUs = 200;
	m_dcqcnPlusInitialCnpOffsetUs = 50;
	m_dcqcnPlusTimerScale = 2.0;
	m_dcqcnPlusAlphaTimerScale = 1.0;
	m_dcqcnPlusCursor = 0;
	m_dcqcnPlusScanArmed = false;
}
...
void RdmaHw::Setup(QpCompleteCallback cb){
	m_dcqcnPlusEnabled = IsScheme("dcqcn_plus");
	Simulator::Cancel(m_dcqcnPlusScanEvent);
	m_dcqcnPlusReceiverTable.clear();
	m_dcqcnPlusReceiverOrder.clear();
	m_dcqcnPlusCursor = 0;
	m_dcqcnPlusScanArmed = false;
	for (uint32_t i = 0; i < m_nic.size(); i++){
		...
		m_nic_datarate_path = GetRunFile("nic_datarate.tr");
		m_nic_cnp_received_path = GetRunFile("nic_cnp_received.tr");
		m_lastCongestionPrintTime = 0;
		m_rtt_path = GetRunFile("rtt.tr");
		m_lastRttTime = 0;
		m_change_parameter_flag = 0;
		m_cnp_received_counter = 0;
	}
...
uint32_t RdmaHw::GetNicIdxOfQp(Ptr<RdmaQueuePair> qp){
	...
	if (v.size() > 0){
		...
		return v[qp->dport % v.size()];     // bonding robin-round
	}else{
...
uint64_t RdmaHw::GetRxQpKey(uint32_t dip, uint16_t pg, uint16_t dport){
...
std::vector<Ptr<RdmaQueuePair>> RdmaHw::GetQpListOnlyByDip(uint32_t dip, uint16_t pg){
	std::vector<Ptr<RdmaQueuePair>> Qp_list;
	for (uint16_t i = 10000; i < 10010; i++)
	{
		uint64_t key = GetQpKey(dip, i, pg);
		auto it = m_qpMap.find(key);
		if (it != m_qpMap.end())
			Qp_list.push_back(it->second);
	}
...
Ptr<RdmaRxQueuePair> RdmaHw::GetRxQp(uint32_t sip, uint32_t dip, uint16_t sport, uint16_t dport, uint16_t pg, bool create){
	uint64_t key = GetRxQpKey(dip, pg, dport);
...
uint32_t RdmaHw::GetNicIdxOfRxQp(Ptr<RdmaRxQueuePair> q){
	...
	if (v.size() > 0){
		...
		//return v[q->dport % v.size()];    // bonding robin-round
	}else{
		NS_ASSERT_MSG(false, "We assume at least one NIC is alive");
	}
}
void RdmaHw::DeleteRxQp(uint32_t dip, uint16_t pg, uint16_t dport){
	uint64_t key = GetRxQpKey(dip, pg, dport);
	RemoveDcqcnPlusReceiverState(key);
	m_rxQpMap.erase(key);
}

void RdmaHw::TrackDcqcnPlusReceiverEcn(Ptr<RdmaRxQueuePair> q, uint8_t ecnbits){
	if (!m_dcqcnPlusEnabled || q == NULL || ecnbits == 0)
		return;

	uint64_t key = GetRxQpKey(q->dip, q->m_ecn_source.qIndex, q->dport);
	auto it = m_dcqcnPlusReceiverTable.find(key);
	if (it == m_dcqcnPlusReceiverTable.end()){
		DcqcnPlusReceiverState state;
		state.rxQp = q;
		state.ecnPending = true;
		state.ecnBits = ecnbits;
		state.lastCnpTime = Simulator::Now() - MicroSeconds(m_dcqcnPlusInitialCnpOffsetUs);
		m_dcqcnPlusReceiverTable[key] = state;
		m_dcqcnPlusReceiverOrder.push_back(key);
	}else{
		it->second.rxQp = q;
		it->second.ecnPending = true;
		it->second.ecnBits |= ecnbits;
	}

	if (!m_dcqcnPlusScanArmed){
		m_dcqcnPlusScanEvent = Simulator::Schedule(MicroSeconds(m_dcqcnPlusScanIntervalUs), &RdmaHw::RunDcqcnPlusReceiverScheduler, this);
		m_dcqcnPlusScanArmed = true;
	}
}

void RdmaHw::RemoveDcqcnPlusReceiverState(uint64_t key){
	m_dcqcnPlusReceiverTable.erase(key);
	for (uint32_t index = 0; index < m_dcqcnPlusReceiverOrder.size(); index++){
		if (m_dcqcnPlusReceiverOrder[index] != key)
			continue;
		m_dcqcnPlusReceiverOrder.erase(m_dcqcnPlusReceiverOrder.begin() + index);
		if (index < m_dcqcnPlusCursor && m_dcqcnPlusCursor > 0)
			m_dcqcnPlusCursor--;
		break;
	}
	if (m_dcqcnPlusReceiverOrder.empty()){
		Simulator::Cancel(m_dcqcnPlusScanEvent);
		m_dcqcnPlusCursor = 0;
		m_dcqcnPlusScanArmed = false;
	}else if (m_dcqcnPlusCursor >= m_dcqcnPlusReceiverOrder.size()){
		m_dcqcnPlusCursor = 0;
	}
}

void RdmaHw::SendDcqcnPlusReceiverCnp(const DcqcnPlusReceiverState &state, uint32_t tauUs, uint32_t entryCount){
	if (state.rxQp == NULL)
		return;

	CnHeader cnh;
	cnh.SetQindex(state.rxQp->m_ecn_source.qIndex);
	cnh.SetFlow(state.rxQp->dport);
	cnh.SetECNBits(state.ecnBits);
	cnh.SetQfb(std::min<uint32_t>(tauUs, 0xffff));
	cnh.SetTotal(std::min<uint32_t>(entryCount, 0xffff));

	Ptr<Packet> newp = Create<Packet>(std::max(60 - 14 - 20 - (int)cnh.GetSerializedSize(), 0));
	newp->AddHeader(cnh);

	Ipv4Header head;
	head.SetDestination(Ipv4Address(state.rxQp->dip));
	head.SetSource(Ipv4Address(state.rxQp->sip));
	head.SetProtocol(0xFF);
	head.SetTtl(64);
	head.SetPayloadSize(newp->GetSize());
	head.SetIdentification(state.rxQp->m_ipid++);

	newp->AddHeader(head);
	AddHeader(newp, 0x800);

	uint32_t nic_idx = GetNicIdxOfRxQp(state.rxQp);
	m_nic[nic_idx].dev->RdmaEnqueueHighPrioQ(newp);
	m_nic[nic_idx].dev->TriggerTransmit();
}

void RdmaHw::RunDcqcnPlusReceiverScheduler(){
	m_dcqcnPlusScanArmed = false;
	if (!m_dcqcnPlusEnabled || m_dcqcnPlusReceiverOrder.empty()){
		m_dcqcnPlusCursor = 0;
		return;
	}
	if (m_dcqcnPlusCursor >= m_dcqcnPlusReceiverOrder.size())
		m_dcqcnPlusCursor = 0;

	uint64_t key = m_dcqcnPlusReceiverOrder[m_dcqcnPlusCursor];
	auto it = m_dcqcnPlusReceiverTable.find(key);
	if (it == m_dcqcnPlusReceiverTable.end()){
		m_dcqcnPlusReceiverOrder.erase(m_dcqcnPlusReceiverOrder.begin() + m_dcqcnPlusCursor);
	}else{
		Time now = Simulator::Now();
		DcqcnPlusReceiverState &state = it->second;
		if (state.ecnPending && (now - state.lastCnpTime) > MicroSeconds(m_dcqcnPlusMinCnpIntervalUs)){
			uint32_t entryCount = m_dcqcnPlusReceiverOrder.size();
			SendDcqcnPlusReceiverCnp(state, entryCount * m_dcqcnPlusScanIntervalUs, entryCount);
			state.ecnPending = false;
			state.ecnBits = 0;
			state.lastCnpTime = now;
			m_dcqcnPlusCursor++;
		}else if ((now - state.lastCnpTime) > MicroSeconds(m_dcqcnPlusEntryTimeoutUs)){
			RemoveDcqcnPlusReceiverState(key);
		}else{
			m_dcqcnPlusCursor++;
		}
...
int RdmaHw::ReceiveUdp(Ptr<Packet> p, CustomHeader &ch){
	...
	if (ecnbits != 0){
		...
		TrackDcqcnPlusReceiverEcn(rxQp, ecnbits);
	}
...
int RdmaHw::ReceiveCnp(Ptr<Packet> p, CustomHeader &ch){
	...
	uint32_t feedbackTauUs = m_dcqcnPlusEnabled ? qfb : 0;

	uint32_t i;
	// get qp
	Ptr<RdmaQueuePair> qp = GetQp(ch.sip, udpport, qIndex);
	if (qp == NULL){
		if (!m_dcqcnPlusEnabled)
	...
	if (m_cc_mode == 1){
		m_cnp_received_counter++;
		cnp_received_mlx(qp, feedbackTauUs);
	}
	return 0;
}
...
int RdmaHw::ReceiveAck(Ptr<Packet> p, CustomHeader &ch){
	...
	if (cnp){
		if (m_cc_mode == 1){ // mlx version
			m_cnp_received_counter++;
			cnp_received_mlx(qp, ch.ack.cnpTau);
		} 
	}

	if (Simulator::Now().GetSeconds() - m_lastRttTime > 1e-4 / 2)
	{
		int rtt = Simulator::Now().GetTimeStep() - ch.ack.ih.ts;

		std::string sip_addr = std::to_string((qp->sip.Get() >> 8) & 0xff);
		std::string dip_addr = std::to_string((qp->dip.Get() >> 8) & 0xff);
		std::string key_name = sip_addr + "-" + dip_addr;
		m_map_last_rtt[key_name] = rtt;

		m_lastRttTime = Simulator::Now().GetSeconds();
	}
...
void RdmaHw::ChangeParameters(std::map<std::string, std::string> parameter_map)
{
	for (auto it : parameter_map)
	{
		std::string parameter_key_str = it.first;
		std::string parameter_value_str = it.second;
		if (parameter_key_str == "rate_to_set_on_first_cnp" || parameter_key_str == "rpg_min_dec_fac" || parameter_key_str == "time_reset" || parameter_key_str == "min_time_between_cnps")
		{
			double parameter_value_double = std::stof(parameter_value_str);
			if (parameter_key_str == "rate_to_set_on_first_cnp")
				m_rateOnFirstCNP = parameter_value_double;
			else if (parameter_key_str == "rpg_min_dec_fac")
				m_min_dec_fac = parameter_value_double;
			else if (parameter_key_str == "time_reset")
				m_rpgTimeReset = parameter_value_double;
			else if (parameter_key_str == "min_time_between_cnps")
				m_rateDecreaseInterval = parameter_value_double;
		}
		else
		{
			uint32_t parameter_value_int = std::stoul(parameter_value_str);
			if (parameter_key_str == "ai_rate")
				m_rai = DataRate(std::to_string(parameter_value_int) + "Mb/s");
			else if (parameter_key_str == "hai_rate")
				m_rhai = DataRate(std::to_string(parameter_value_int) + "Mb/s");
			else if (parameter_key_str == "rpg_min_rate")
				m_minRate = DataRate(std::to_string(parameter_value_int) + "Mb/s");
			else if (parameter_key_str == "rpg_gd")
				m_rpg_gd = parameter_value_int;
			else if (parameter_key_str == "dce_tcp_g")
				m_dce_tcp_g = parameter_value_int;
			else if (parameter_key_str == "initial_alpha_value")
				m_initialAlphaValue = parameter_value_int;
			
		}
	}
}

void RdmaHw::ClearRttMap()
{
	m_map_last_rtt.clear();
}
...
void RdmaHw::UpdateAlphaMlx(Ptr<RdmaQueuePair> q){
	...
	#endif
	if (m_dcqcnPlusEnabled){
		q->mlx.m_alpha = std::max(0.0, (1.0 - m_g) * q->mlx.m_alpha);
		ScheduleUpdateAlphaMlx(q);
		return;
	}
	if (q->mlx.m_alpha_cnp_arrived){
		q->mlx.m_alpha = m_dce_tcp_g/std::pow(2, 10)*q->mlx.m_alpha + std::pow(2, 10)-m_dce_tcp_g;
	}else {
		q->mlx.m_alpha = m_dce_tcp_g/std::pow(2, 10)*q->mlx.m_alpha;
	}
	#if PRINT_LOG
	#endif
...
void RdmaHw::ScheduleUpdateAlphaMlx(Ptr<RdmaQueuePair> q){
	if (m_dcqcnPlusEnabled){
		q->mlx.m_eventUpdateAlpha = Simulator::Schedule(MicroSeconds(q->mlx.m_alphaInterval), &RdmaHw::UpdateAlphaMlx, this, q);
		return;
	}
	q->mlx.m_eventUpdateAlpha = Simulator::Schedule(MicroSeconds(m_alpha_resume_interval), &RdmaHw::UpdateAlphaMlx, this, q);
}

void RdmaHw::cnp_received_mlx(Ptr<RdmaQueuePair> q, uint32_t feedbackTauUs){
	if (m_dcqcnPlusEnabled){
		uint32_t nic_idx = GetNicIdxOfQp(q);
		Ptr<QbbNetDevice> dev = m_nic[nic_idx].dev;
		DataRate currentRate = q->m_rate;
		if (q->mlx.m_first_cnp)
			q->mlx.m_alpha = m_initialAlphaValue;
		q->mlx.m_alpha = std::min(1.0, (1.0 - m_g) * q->mlx.m_alpha + m_g);
		double packet_tx_time_us = 0.0;
		uint32_t packet_bytes = std::max<uint32_t>(m_mtu == 0 ? 1000 : m_mtu, q->lastPktSize == 0 ? 1000 : q->lastPktSize);
		if (currentRate.GetBitRate() > 0)
			packet_tx_time_us = (packet_bytes * 8.0 * 1e6) / currentRate.GetBitRate();
		double reset_interval_us = std::max<double>(std::max<uint32_t>(m_dcqcnPlusMinTauUs, feedbackTauUs), packet_tx_time_us);
		q->mlx.m_feedbackTau = std::max<uint32_t>(m_dcqcnPlusMinTauUs, feedbackTauUs);
		q->mlx.m_alphaInterval = std::max(1.0, m_dcqcnPlusAlphaTimerScale * reset_interval_us);
		q->mlx.m_recoveryInterval = std::max(1.0, m_dcqcnPlusTimerScale * reset_interval_us);
		q->mlx.m_targetRate = currentRate;
		double new_rate_bps = std::max<double>(m_minRate.GetBitRate(), currentRate.GetBitRate() * (1.0 - q->mlx.m_alpha / 2.0));
		ChangeRate(q, DataRate((uint64_t)std::max(1.0, new_rate_bps)));
		q->mlx.m_rpTimeStage = 0;
		q->mlx.m_first_cnp = false;
		Simulator::Cancel(q->mlx.m_eventUpdateAlpha);
		Simulator::Cancel(q->mlx.m_rpTimer);
		ScheduleUpdateAlphaMlx(q);
		q->mlx.m_rpTimer = Simulator::Schedule(MicroSeconds(q->mlx.m_recoveryInterval), &RdmaHw::RateIncEventTimerMlx, this, q);
		return;
	}
	...
	if (q->mlx.m_first_cnp){
		...
		q->mlx.m_targetRate = q->m_rate = m_rateOnFirstCNP * q->m_rate;   // rate_to_set_on_first_cnp
		q->mlx.m_first_cnp = false;
	}
}
...
void RdmaHw::CheckRateDecreaseMlx(Ptr<RdmaQueuePair> q){
	...
	if (q->mlx.m_decrease_cnp_arrived){
		...
		q->m_rate = std::max({m_minRate, m_min_dec_fac*q->m_rate, q->m_rate * (1 - q->mlx.m_alpha / std::pow(2, m_rpg_gd))});
...
void RdmaHw::ActiveIncreaseMlx(Ptr<RdmaQueuePair> q){
	...
	Ptr<QbbNetDevice> dev = m_nic[nic_idx].dev;
	if (m_dcqcnPlusEnabled){
		double line_rate_bps = dev->GetDataRate().GetBitRate();
		double current_rate_bps = q->m_rate.GetBitRate();
		double step_bps = q->mlx.m_alpha > 0.1 ? std::min(current_rate_bps / 5.0, line_rate_bps / 50.0) : std::min(current_rate_bps / 10.0, line_rate_bps / 100.0);
		double target_rate_bps = std::min(line_rate_bps, q->mlx.m_targetRate.GetBitRate() + step_bps);
		q->mlx.m_targetRate = DataRate((uint64_t)std::max(1.0, target_rate_bps));
		ChangeRate(q, (q->m_rate / 2) + (q->mlx.m_targetRate / 2));
		return;
	}
	// increate rate
	//std::cout << Simulator::Now().GetSeconds() << ", ai, m_node = " << m_node->GetId() << ", dport=" << q->dport << "\n";
...
void RdmaHw::HyperIncreaseMlx(Ptr<RdmaQueuePair> q){
	...
	if (m_dcqcnPlusEnabled){
		double line_rate_bps = dev->GetDataRate().GetBitRate();
		double current_rate_bps = q->m_rate.GetBitRate();
		double step_scale = std::max(1.0, double(q->mlx.m_rpTimeStage - 4 * m_rpgThreshold + 1));
		double step_bps = std::min(current_rate_bps, step_scale * line_rate_bps / 100.0);
		double target_rate_bps = std::min(line_rate_bps, q->mlx.m_targetRate.GetBitRate() + step_bps);
		q->mlx.m_targetRate = DataRate((uint64_t)std::max(1.0, target_rate_bps));
		ChangeRate(q, (q->m_rate / 2) + (q->mlx.m_targetRate / 2));
		return;
	...
	// increate rate
	//std::cout << Simulator::Now().GetSeconds() << ", hai, m_node = " << m_node->GetId() << ", dport=" << q->dport << "\n";
