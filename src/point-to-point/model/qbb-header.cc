...
namespace ns3 {
	...
	qbbHeader::qbbHeader(uint16_t pg)
		: m_pg(pg), sport(0), dport(0), flags(0), m_seq(0), m_cnpTau(0)
	{
	}

	qbbHeader::qbbHeader()
		: m_pg(0), sport(0), dport(0), flags(0), m_seq(0), m_cnpTau(0)
	{}
	...
	void qbbHeader::SetCnpTau(uint32_t tau){
		m_cnpTau = tau;
	}
	...
	uint32_t qbbHeader::GetCnpTau() const{
		return m_cnpTau;
	}
	...
	uint32_t qbbHeader::GetBaseSize() {
		qbbHeader tmp;
		return sizeof(tmp.sport) + sizeof(tmp.dport) + sizeof(tmp.flags) + sizeof(tmp.m_pg) + sizeof(tmp.m_seq) + sizeof(tmp.m_cnpTau);
	}
