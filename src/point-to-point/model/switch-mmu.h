...
public:
	...
	uint64_t GetPortEgressBytes(uint32_t port) const;
	uint32_t GetPortKmin(uint32_t port) const;
	uint32_t GetPortKmax(uint32_t port) const;
	double GetPortPmax(uint32_t port) const;
