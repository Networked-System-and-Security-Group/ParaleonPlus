...
namespace ns3 {
	...
	uint64_t SwitchMmu::GetPortEgressBytes(uint32_t port) const{
		uint64_t total = 0;
		for (uint32_t qIndex = 1; qIndex < qCnt; qIndex++)
			total += egress_bytes[port][qIndex];
		return total;
	}
	uint32_t SwitchMmu::GetPortKmin(uint32_t port) const{
		return kmin[port] / 1000;
	}
	uint32_t SwitchMmu::GetPortKmax(uint32_t port) const{
		return kmax[port] / 1000;
	}
	double SwitchMmu::GetPortPmax(uint32_t port) const{
		return pmax[port];
	}
}
