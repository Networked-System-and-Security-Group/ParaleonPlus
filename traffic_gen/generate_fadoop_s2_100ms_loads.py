...
def parse_args():
    parser = argparse.ArgumentParser(description="Generate Fadoop s2 workloads for multiple host-rate loads")
    parser.add_argument("--loads", default="70,80", help="comma-separated per-host loads in Gbps")
    parser.add_argument("--duration-ms", type=float, default=100.0, help="traffic duration in milliseconds")
    parser.add_argument("--base-time-s", type=float, default=2.0, help="base start time in seconds")
    parser.add_argument("--seed-base", type=int, default=36000, help="base random seed; each load uses seed_base + load")
...
def generate_one(load_gbps, duration_s, base_time_s, seed_base):
    cdf_path = SCRIPT_DIR / "FbHdp.txt"
    output_path = SCRIPT_DIR / ("flow_fadoop_s2_100ms_%dG.txt" % load_gbps)
